...
- `label` — ข้อความหลัง `]` จนถึง `[` ตัวถัดไปหรือ SEP (ตัด `+` ที่หัว/ท้ายกับช่องว่างออก) ว่างได้
  - `+` จบ block ก็ต่อเมื่อมี `[` ตามหลัง — `+` ที่ไม่มี `[` ตามหลังนับเป็นส่วนหนึ่งของคำอธิบาย เช่น `ขนม: [100] อาหาร + เสื้อ` → label เดียวคือ `อาหาร + เสื้อ`
  - `+` ค้างท้ายที่ไม่มี block ตามมา (เช่น `ขนม: [100] อาหาร +`) ถูกตัดทิ้งเงียบๆ ไม่นับเป็น `invalid` — ระหว่างพิมพ์ใน live preview สถานะนี้เกิดตลอด การเตือนจึงรบกวนมากกว่าช่วย
